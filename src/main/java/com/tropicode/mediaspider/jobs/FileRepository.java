...
public class FileRepository {
    ...
    private MediaPath registerPaths(MediaFile file) {
        ...
        int pathHash = parentPath.toAbsolutePath().toString().hashCode();
        MediaPath mediaPath = pathMap.get(pathHash);
        if (mediaPath == null) {
            mediaPath = new MediaPath(parentPath);
            pathMap.put(pathHash, mediaPath);
        } else {
            mediaPath.incrementFileCount();
        }
        return mediaPath;
    }
